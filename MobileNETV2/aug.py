...
def train_mnist():
    ...
    for i in range(len(a)):
        if a[i] == a[0]:
            ...
            d1.append([b[i][0], (max_cnt-a[i])]) 
            d2.append([b[i][0], (max_cnt-a[i])//20])
    ...
    for k in range(20):
        with torch.no_grad():
            ...
            combined_indices = []
            for i in range(4):
                combined_indices.extend([idx0[i], idx1[i], idx2[i]])

            for gi in range(len(x_gen)):
                x_gen[gi] = WT(x_gen[gi].flatten())
            
            
            x_gen = np.pad(x_gen, ((0, 0), (8, 8)), 'constant', constant_values=0)
            x_gen[:,:8] = x_gen[:, 8:9]
            x_gen[:, -8:] = x_gen[:, -9:-8]
            y_gen += 1
            np.save(save_dir + 'diff_data_' + str(k), x_gen) 
            np.save(save_dir + 'diff_label_' + str(k), y_gen.detach().cpu().numpy()) 
